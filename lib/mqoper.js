 ...
 * msg: msg to send
 * callback: callback for reply msg
...
MqOper.prototype.connectReadyForService = function (serviceName, recvCallback) {
    ...
    connect.on('ready', function() {
        console.log((new Date()) + ' Connect to MQ for %s is ready: %s', serviceName, JSON.stringify(mqOption));
        ...
        connect.queue(serviceName, cons.queueParas.qOption, function (q1) {
            console.log("Queue "+serviceName+" for reply is open.");
...
MqOper.prototype.connectReadyForServer = function (recvCallback) {
    ...
    var connect = amqp.createConnection(mqOption);
    connect.on('ready', function() {
        console.log((new Date()) + ' Connect to MQ for %s is ready: %s', cons.serviceName.webserver, JSON.stringify(mqOption));
        ...
        connect.queue(pOption.replyTo, cons.queueParas.qOption, function (q1) {
            console.log("Queue "+pOption.replyTo+" for reply is open.");
