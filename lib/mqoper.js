...
MqOper.prototype.sendMsg = function (pkey, msg, callback) {
    ...
    {
        console.error("[MQHD]Exchange "+sendExchage.name+" not around now!");
    }
};
...
//    this.setSendOption('replyTo', serviceName);
    ...
    connect.on('error', function (err) {
        console.error("[MQHD]Failed to connect MQ for "+serviceName+': '+err.name+', '+err.message);
    });
...
MqOper.prototype.connectReadyForServer = function (recvCallback) {
    ...
    connect.on('error', function (err) {
        console.error('[MQHD]Failed to connect MQ for webserver: '+err.name+', '+err.message);
    });
