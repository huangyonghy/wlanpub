...
MqOper.prototype.sendMsg = function (pkey, msg, callback) {
    ...
    if (option.appId == cons.serviceName.webserver && cons.bIsValidService(pkey, true)) {
...
//    this.setSendOption('replyTo', serviceName);
    ...
    connect.on('error', function (err) {
        console.error("[MQHD]Failed to connect MQ for "+serviceName+': '+err.message+', mq option: '+JSON.stringify(mqOption));
    });
...
MqOper.prototype.connectReadyForServer = function (recvCallback) {
    ...
    connect.on('error', function (err) {
        console.error('[MQHD]Failed to connect MQ for webserver: '+err.message+', mq option: '+JSON.stringify(mqOption));
    });
    connect.on('end', function (err) {
        console.warn("[MQHD]received end event.");
    });
