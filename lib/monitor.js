...
function Monitor() {
  ...
  this._pid = 0;
  this._instanceName = '';
  this._keyTailHost  = ':HOSTNAME';   // ip map hostname
  this._hostname = '';
...
//    this.redisClient = new redis.Cluster([para]);
  ...
  this.redisClient.on('error', function (err) {
    console.error('[MONIT]Failed to create monitor client for '+moduleName+' with '+err+', redis option: '+JSON.stringify(para));
  });
};
...
Monitor.prototype.accessStats = function(req, res, next){
  ...
  if (('' != name) && (undefined != name)) {
    ...
    if (isValidService) {
      ...
      monitor._httpReqResTime(req.reqID, monitor.sectMark.req);
      ...
      res.on('finish', function() {
        ...
        monitor._httpReqResTime(req.reqID, monitor.sectMark.res);
      });
    }
  }
...
Monitor.prototype._getHostname = function (pid, ipv4, callback) {
  ...
  var ip2hostKey = monitor._comPrefix + subIP + monitor._keyTailHost;

  redis.get(ip2hostKey, function (error, hostname) {
    if (null == hostname) {
      hostname = '';

      console.error('[MONIT]Failed to get hostname for ip '+ipv4);
    }
