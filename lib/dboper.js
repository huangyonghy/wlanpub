...
DbOper.prototype.connectRedis = function(redisPara) {
    ...
    this.redisClient.on("connect", function(){
        console.log((new Date()) + ' database connected to redis success: ' + JSON.stringify(para));
    });
    this.redisClient.on("end", function(){
        console.log((new Date()) + ' database end connection to redis.');
    });
...
DbOper.prototype.connectMongoose = function(mongoosePara) {
    ...
    this.mongo.on('connected', function() {
        console.log((new Date()) + ' database connected to mongoose success：' + para);});
    this.mongo.on('disconnected', function() {
        console.log((new Date()) + ' database disconnected to mongoose.');});
    this.mongo.on('error', function(error) {
        console.log((new Date()) + ' database connected to mongoose with error: ' + error);
    });
};
